...
class GuardianData():
    ...
    def input_data(self, level=5, guardian_type="ソロ", guardian_class="ミーレス（カバリエ）", weapon_var=["白兵", "射撃", "魔導"], weapon_num="4"):
        ...
        weapon_num_int = int(weapon_num)
        got_weapon_num = 0
        print(weapon_var)
        maxloop = 1000
        loop = 0

        while got_weapon_num < weapon_num_int and loop < maxloop:
            print("max_weapon_num = " + str(weapon_num))
            print("weapon_num = " + str(got_weapon_num))
            ...
            if "白兵" in weapon_var and weapon_weapon_type == "白兵" and main_weapon_short_flg == False:
                ...
                got_weapon_num = got_weapon_num + 1
...
def get_data(level=3, guardian_type="ソロ", guardian_class="モンスター", weapon_var=[], weapon_num="4"):
    weapon_var_list = []
    ...
    print(weapon_var)
    print("weapon_number = " + str(weapon_num))
    print(weapon_var_list2)

    guardian.input_data(level=level, guardian_type=guardian_type, guardian_class=guardian_class, weapon_var=weapon_var_list2, weapon_num=weapon_num)
...
if __name__ == "__main__":
    ...
    # ラベル
    Static3 = tkinter.Label(frame9, text=u'武装の数')
    Static3.pack()
    ...
    Button1 = tkinter.Button(frame11, text=u'生成', command=lambda: [get_data(int(EditBox.get()), ComboBox1.get(), ComboBox2.get(), checkBoxVars, ComboBox3.get())])
    Button1.pack()

    # ボタン
    Button2 = tkinter.Button(frame12, text=u'終了', command=lambda: root.quit())
    Button2.pack()
