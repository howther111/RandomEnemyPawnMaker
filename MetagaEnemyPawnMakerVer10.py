...
class GuardianData():
    ...
    def input_data(self, level=5, guardian_type="ソロ", guardian_class="奈落獣", weapon_var=["白兵", "射撃", "砲撃"], weapon_num="4"):
...
def get_data(level=3, guardian_type="ソロ", guardian_class="ミーレス（カバリエ）", weapon_var=[], weapon_num="4"):
    weapon_var_list = []
...
if __name__ == "__main__":
    ...
    Button1 = tkinter.Button(frame11, text=u'生成',
                             command=lambda: [get_data(int(EditBox.get()), ComboBox1.get(), ComboBox2.get(), checkBoxVars, ComboBox3.get())])
    Button1.pack()
